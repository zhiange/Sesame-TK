                        ...
                        if (retBubbles != null) {
                            for (int j = 0; j < retBubbles.length(); j++) {
                                ...
                                if (retBubble != null) {
                                    ...
                                    LogUtil.forest("神奇海洋🐳收取[" + UserIdMap.getMaskName(userId) + "]的海洋能量#"
                                            + collectedEnergy + "g");
                                }
                            }
                        }
        ...
        try {
            for (int i = 0; i < rubbishNumber; i++) {
                ...
                if ("SUCCESS".equals(jo.getString("resultCode"))) {
                    ...
                    LogUtil.forest("神奇海洋🐳[清理:" + UserIdMap.getMaskName(userId) + "海域]");
                } else {
                    LogUtil.runtime(TAG, jo.getString("resultDesc"));
                }
            }
    ...
    private void cleanFriendOcean(JSONObject fillFlag) {
        ...
        try {
            ...
            if ("SUCCESS".equals(jo.getString("resultCode"))) {
                ...
                jo = new JSONObject(s);
                LogUtil.forest("神奇海洋🐳[帮助:" + UserIdMap.getMaskName
                        (userId) + "清理海域]");
                if ("SUCCESS".equals(jo.getString("resultCode"))) {
                    JSONArray cleanRewardVOS = jo.getJSONArray("cleanRewardVOS");
                    checkReward(cleanRewardVOS);
